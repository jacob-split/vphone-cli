...
- (NSDictionary * _Nullable)serializeCompactLeafElement:(AXUIElementRef)leafElement
    ...
    NSString *identifier = MCPAXNodeStringFromValue(xcValues[@"identifier"]);
    NSString *placeholder = MCPAXNodeStringFromValue(xcValues[@"placeholderValue"]);
    if (identifier.length > 0) element[@"identifier"] = identifier;
    if (placeholder.length > 0) element[@"placeholder"] = placeholder;
    id traits = MCPAXNodeNormalizedValue(xcValues[@"traits"]);
    if (traits) element[@"traits"] = traits;
    NSNumber *interaction = MCPAXNodeNumberFromValue(xcValues[@"isUserInteractionEnabled"]);
    if (interaction) element[@"user_interaction_enabled"] = @([interaction boolValue]);
    NSNumber *automationType = MCPAXNodeNumberFromValue(xcValues[@"automationType"]);
    if (automationType) element[@"automation_type"] = automationType;
    id containerType = MCPAXNodeNormalizedValue(directValues[@"containerType"]);
...
- (NSDictionary * _Nullable)treeForPid:(pid_t)pid
                                  bundleId:(NSString * _Nullable)bundleId
                                 contextId:(uint32_t)contextId
                                 displayId:(uint32_t)displayId
                                  maxDepth:(NSInteger)maxDepth
    ...
    [self.attributeBridge performOnMainThreadSync:^{
        @try {
            ...
            NSInteger boundedDepth = MAX(0, MIN(maxDepth > 0 ? maxDepth : 12, 60));
            NSInteger boundedElements = MAX(1, MIN(maxElements > 0 ? maxElements : 500, 2000));
            NSInteger count = 0;
            NSMutableSet<NSString *> *visited = [NSMutableSet set];
            NSDictionary *root = [self serializeRemoteElement:appElement
                                                        depth:0
                                                     maxDepth:boundedDepth
                                                        count:&count
                                                  maxElements:boundedElements
                                                      visited:visited];
            CFRelease(appElement);
            if (![root isKindOfClass:[NSDictionary class]]) {
                resultError = @"Direct AX tree serialized no root element";
                return;
            }

            NSMutableDictionary *payload = [NSMutableDictionary dictionary];
            payload[@"format"] = @"tree";
            payload[@"source"] = @"direct_ax_recursive_tree";
            payload[@"pid"] = @(pid);
            ...
            payload[@"max_depth"] = @(boundedDepth);
            payload[@"max_elements"] = @(boundedElements);
            payload[@"element_count"] = @(count);
            payload[@"root"] = root;
            resultPayload = payload;
        } @catch (NSException *exception) {
            resultError = [NSString stringWithFormat:@"AX tree exception: %@: %@",
                           exception.name, exception.reason ?: @"<no reason>"];
        }
    }];
